...
def main():
    ...
    if period == "morning":
        ...
        ai_prompt = f"Сегодня {current_date}. Данные: {weather_context}. Ты — ведущий синоптик. Проанализируй синоптическую карту над Пинском: определи тип воздушной массы и систему (циклон/антициклон и их название). Не называй цифры, вместо этого опиши, как погода реально ощущается на улице (сырость, резкость ветра, мягкость воздуха). Объясни причину такой погоды и чего ждать в ближайшее время (1-2 суток). 2-3 предложения. Дай совет."
    elif period == "day":
        ...
        ai_prompt = f"Сегодня {current_date}. Обед. Данные: {weather_context}. Утром было: {history_str}. Ты — синоптик. Глядя на карты, опиши, как сместились фронты. Расскажи, как изменились ощущения за окном по сравнению с утром (стало ли легче дышать, стал ли ветер «злее» и т.д.). Не используй цифры. Дай прогноз до вечера 1-2 предложениями."
    else:
        ...
        ai_prompt = f"Сегодня {current_date}. Вечер. Сейчас: {weather_context}. Днем: {history_str}. Ночью будет {min(night_temps)}°C, осадки: {night_prec}. Ты — синоптик. Объясни характер ночной погоды через карты (адвекция холода, влияние фронта). Опиши, какой воздух будет ночью на ощупь по сравнению с дневным. Кратко 1-2 предложения."
