...
def main():
    ...
    if 4 <= hour <= 8:
        ...
        ai_prompt = f"Ты метеоролог. Дай глубокую АНАЛИТИКУ движения воздушных масс (циклон,антициклон с названием , физические явления, данные бери в интернете) и как это повлияет на погоду предстоящего дня для Пинска. Без цифр,символов,кратко(1-2 предложения).Пиши сразу по существу, без вводных фраз и заголовков"
    ...
    # --- ДНЕВНАЯ ДЕЖУРКА (14:00) ---
        ...
        ai_prompt = f"Ты метеоролог.Сравни текущую погоду ({cur['temperature_2m']}°C) с утренней ({prev}°C) в Пинске. Объясни изменения  и как изменилась температура и погода. Кратко в одно предложение.Если нет каких-то данных просто сообщи.Пиши сразу по существу, без вводных фраз и заголовков."
    ...
    # --- ВЕЧЕРНЯЯ СВОДКА (20:00) ---
        ...
        ai_prompt = f"Ты метеоролог.Сравни вечер ({cur['temperature_2m']}°C) с днем ({prev}°C) в Пинске. Объясни изменения и как изменилась температура и погода. Кратко одним предложением.Если нет каких-то данных просто сообщи.Пиши сразу по существу, без вводных фраз и заголовков"
