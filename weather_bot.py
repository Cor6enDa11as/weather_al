...
def main():
    ...
    if 4 <= hour <= 8:
        ...
        ai_prompt = f"Ты метеоролог. Дай глубокую АНАЛИТИКУ движения воздушных масс (циклон,антициклон с названием , физические явления, данные бери в интернете) и как это повлияет на погоду предстоящего дня для Пинска. Без цифр,символов и анотаций, кратко(1-2 предложения)."
    ...
    # --- ДНЕВНАЯ ДЕЖУРКА (14:00) ---
        ...
        ai_prompt = f"Ты метеоролог.Сравни текущую погоду ({cur['temperature_2m']}°C) с утренней ({prev}°C) в Пинске. Объясни изменения (прогрев, облака, ветер,давление и т.д,магнитный фон если нужно) и как изменилась температура. Кратко в одно предложение.Если нет каких-то данных просто сообщи.Вводных слов, символов и анотаций не пиши , только по делу."
    ...
    # --- ВЕЧЕРНЯЯ СВОДКА (20:00) ---
        ...
        ai_prompt = f"Ты метеоролог.Сравни вечер ({cur['temperature_2m']}°C) с днем ({prev}°C) в Пинске. Объясни изменения (прогрев, облака, ветер,давление и т.д ,магнитный фон если нужно) и как изменилась температура. Кратко одним предложением.Если нет каких-то данных просто сообщи.Вводных слов,символов и анотаций не пиши , только по делу."
