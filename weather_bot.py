...
# --- Функции пояснения (Те самые подсказки) ---
...
def ask_ai_cascade(prompt_msg, system_preamble):
    if COHERE_KEY:
        ...
        except Exception as e: log(f"❌ Cohere error: {e}")

    if GEMINI_KEY:
        try:
            log("🤖 Запрос к Gemini...")
            # Оставляю gemini-3-flash по твоему требованию
...
def main():
    ...
    for i in range(idx_now, idx_now + 18):
        ...
        if v > 0.1 or prob > 20:
            precip_info = f"{get_weather_desc(h['weather_code'][i])} ({prob}%) ожидается около {i % 24:02d}:00"
            break

    # --- Уровни опасности (Только Оранж и Красный) ---
    danger_alerts = []
    kp_val = 0
    try:
    ...
    if 5 <= hour < 14:
        ...
        preamble = ("Ты — главный синоптик смены. Подведи итог суточного энергообмена. Твоя задача: на основе радиационного баланса(облачность) и влажности определить характер ночи и утра.Спрогнозируй тип конденсации: при отрицательных температурах — гололедица или иней; при положительных — туман или роса.Оцени, как барический тренд(падение или рост давления) и ночное остывание изменят погоду и скажутся на качестве сна и самочувствии ночью и утром.ПРАВИЛА:Экспертный вердикт понятный простому человеку.3-4 ёмких предложения.")
    ...
    if hour >= 20 and dow in [2, 6]:
        ...
        for i in range(4, 7):
            ...
            # Логика уровней для стратегии
            day_alert_str = ""
            if g_max >= 54 or t_max >= 30: day_alert_str = "\n\n⚠️ **Уровень:**\n🟠 ОРАНЖЕВЫЙ"
            if g_max >= 90: day_alert_str = "\n\n⚠️ **Уровень:**\n🔴 КРАСНЫЙ"

            blocks.append(f"📅 **{dt}**\n\n🌡 **Температура:**\n{t_min}..{t_max}°C\n\n🌧 **Осадки:**\n{p_sum} мм ({p_prob}%)\n\n🌬 **Ветер:**\n{w_max} км/ч (порывы {g_max}){day_alert_str}")
