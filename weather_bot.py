...
def main():
    ...
    # Формируем отметки: для ночи (00, 06) и для утра/дня (09, 15)
    def get_point(h_target):
        idx = h_target + 24 if hour > 20 and h_target < 12 else h_target
        if idx < len(hr['temperature_2m']):
            return f"{h_target:02d}:00({hr['temperature_2m'][idx]}°C, ощущ.{hr['apparent_temperature'][idx]}°C, ветер {hr['wind_speed_10m'][idx]}км/ч, {get_weather_desc(hr['weather_code'][idx])})"
        return ""

    night_points = f"{get_point(0)}, {get_point(6)}"
    day_points = f"{get_point(9)}, {get_point(15)}"

    air_status = get_aqi_status(pm25)
    mag_status = get_mag_status(kp)

    # Базовая строка данных (без завтрашнего диапазона и заката)
    data_str = (f"Пинск сейчас: {cur['temperature_2m']}°C (ощущ. {cur['apparent_temperature']}°C). "
                ...
                f"Воздух: {air_status}. Магнит: {mag_status}.")
