...
def log(message):
    # Лог теперь тоже показывает время по Пинску для удобства проверки
...
def main():
    ...
    # КОРРЕКЦИЯ ВРЕМЕНИ: Добавляем 3 часа к UTC для Пинска
    ...
    # ИСПРАВЛЕННЫЙ Kp-индекс через JSON API (NOAA)
    kp = 0.0
    try:
        log("🧲 Получение Kp-индекса (JSON API)...")
        kp_res = requests.get("https://services.swpc.noaa.gov/products/noaa-scales.json", timeout=10).json()
        # Достаем текущее значение Kp из JSON структуры
        kp = float(kp_res['0']['mag_eff']['kp'])
        log(f"✅ Kp-индекс получен: {kp}")
    except Exception as e:
        log(f"⚠️ Ошибка Kp (JSON): {e}")
