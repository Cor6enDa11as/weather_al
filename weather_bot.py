...
def main():
    ...
    # --- ДНЕВНАЯ ДЕЖУРКА (14:00) ---
        ...
        ai_prompt = f"Ты метеоролог.Сравни текущую погоду ({cur['temperature_2m']}°C) с утренней ({prev}°C) в Пинске. Объясни изменения (прогрев, облака, ветер,давление и т.д,магнитный фон если нужно) и как изменилась температура. Кратко в одно предложение.Если нет каких-то данных просто сообщи."
    ...
    # --- ВЕЧЕРНЯЯ СВОДКА (20:00) ---
        ...
        ai_prompt = f"Ты метеоролог.Сравни вечер ({cur['temperature_2m']}°C) с днем ({prev}°C) в Пинске. Объясни изменения (прогрев, облака, ветер,давление и т.д ,магнитный фон если нужно) и как изменилась температура. Кратко одним предложением.Если нет каких-то данных просто сообщи."
